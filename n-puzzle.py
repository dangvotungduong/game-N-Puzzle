...
def center_window(root, width, height): # Đảm bảo cửa sổ trò chơi xuất hiện ở trung tâm màn hình.
...
def prepare_puzzle(): # Chọn số hàng số cột và đồng thời khởi tạo trò chơi
    global ROW, COL
...
def main_game(ROW, COL):
    ...
    SIZE = int(300 / ROW)
    puzzle = list(range(0, ROW * COL)) #Trạng thái ban đầu của bảng
    saved_state = list(puzzle) #Trạng thái lưu trữ
    goal = list(range(0, ROW * COL)) #Trạng thái mục tiêu

    def create_square_image(): #Tạo hình nền ô vuông mặc định
        square_image = Image.new("RGB", (100, 100), "#0666a1")
        return square_image

    def is_solved(puzzle): #Kiểm tra trạng thái giải xong
        return puzzle == goal
    
    def update_display(): #Cập nhật giao diện khi thay đổi trạng thái puzzle
        stock = ROW * COL + 1
        for i in range(ROW):
            for j in range(COL):
                button = buttons[i][j]
    ...
    def move(puzzle, move_to): # Hàm move và on_button_click xử lý việc di chuyển ô trống khi người chơi nhấn nút hoặc dùng phím (on_key_press)
        global step_count
    ...
    def possible_moves(current_node): #Tạo danh sách các trạng thái có thể di chuyển từ trạng thái hiện tại
        moves = []
    ...
    def random_shuffle(puzzle): #Xáo trộn puzzle
        visited = set()
    ...
    def state_to_string(state): #Giúp chuyển đổi trạng thái trò chơi thành một chuỗi để hiển thị.
        puzzle_string = ""
        for i in range(ROW):
            for j in range(COL):
    ...
    def reset_infor_lables(): #Giúp đặt lại tất cả các giá trị về trạng thái ban đầu.
    ...
    def update_depth_limit_count(count): # Cập nhật nhãn hiển thị số lượng giới hạn độ sâu
        depth_limit_label.config(text=f"{count}")

    def update_step_count(count): #Cập nhật nhãn hiển thị số bước di chuyển đã thực hiện 
        step_label.config(text=f"{count}")

    def update_total_steps_count(count): #Cập nhật nhãn hiển thị tổng số bước đã thực hiện 
        total_steps_label.config(text=f"{count}")

    def update_total_nodes_count(count): #Cập nhật nhãn hiển thị tổng số nút
        total_nodes_label.config(text=f"{count}")

    def update_solving_time(count): # Sẽ cập nhật nhãn thời gian giải quyết trò chơi với thời gian làm tròn đến 2 chữ số thập phân.
        time_label.config(text=f"{count:.2f}s")

    def update_state_lable(str_saved_state, str_goal): #Sẽ cập nhật nhãn hiển thị trạng thái trò chơi hiện tại và mục tiêu
        if ROW > 3 or COL > 3:
...
def btn_solution_click(): # Hiển thị hướng dẫn giải dưới dạng số hoặc hướng (L, R, U, D) từ các file lưu trữ
...
def btn_history_click(): # Đọc và hiển thị lịch sử giải từ file history.txt
...
def btn_change_size(): # Tải lại chương trình với kích thước puzzle mới.
    game_window.destroy()
    delete_file()
    os.system('python {}'.format(' '.join(sys.argv)))


def btn_exit_click(): # Dừng chương trình và xóa dữ liệu lịch sử.
    delete_file()
    sys.exit()


def delete_file(): # Xoá flie lịch sử giải và hướng dẫn giải
...
def main_menu(): #Giao diện menu
    ...
    tk.Label(menu_frame, text="Nhóm 05", 
             fg="black", font=("Tahoma", 25, "bold")).grid(
                 row=0, column=0, padx=25, pady=15
    )
    tk.Label(menu_frame, text="Đặng Võ Tùng Dương - 2254810311", 
